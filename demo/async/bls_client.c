...
int
client(const char *clientstr, const char *url, const char *msecstr)
{
	nng_socket sock;
	int        rv;
	nng_msg *  msg;
	...
	printf("Request took %u milliseconds for client %u.\n"
            , (uint32_t)(end - start)
            , client_num
            );
	return (0);
...
main(int argc, char **argv)
{
	...
	if (argc != 4) {
		fprintf(stderr, "Usage: %s <clientNum> <url> <secs>\n", argv[0]);
		exit(EXIT_FAILURE);
	}
